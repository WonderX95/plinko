...
const FundedHomePage = () => {
  ...
  useEffect(() => {
    ...
    const zoneY = containerHeight - zoneHeight / 2 - 2;

    // Calculate total width available
    const totalWidth = containerWidth * 0.98; // Use 98% of width to allow for small gaps

    // Allocate widths based on multiplier values
    const getZoneWidth = (multiplier: string) => {
      if (multiplier === "10x") {
        return totalWidth * 0.165; // 16.5% for 10x multipliers
      } else if (multiplier === "0.7" || multiplier === "0.9") {
        return totalWidth * 0.07; // Increased to 7% for middle multipliers
      } else if (multiplier === "5x") {
        return totalWidth * 0.075; // Reduced to 7.5%
      } else if (multiplier === "2x") {
        return totalWidth * 0.07; // Reduced to 7%
      } else if (multiplier === "1.5") {
        return totalWidth * 0.07; // Reduced to 7%
      } else {
        return totalWidth * 0.055; // Reduced to 5.5% for smaller multipliers
      }
    };

    // Calculate starting positions
    let currentX = containerWidth * 0.01; // Start at 1% of container width

    multipliers.forEach((multiplier, index) => {
      const zoneWidth = getZoneWidth(multiplier);
      const zoneX = currentX + zoneWidth / 2;

      // Update for next zone with a small gap
      currentX += zoneWidth + 1; // Add 1px gap between zones
      ...
      // Create custom chamfer for 10x multipliers
      let chamfer = { radius: 6 }; // Default for most multipliers
      ...
      if (multiplier === "10x") {
        if (index === 0) {
          ...
          chamfer = {
            topRight: 20,
            bottomRight: 20,
            topLeft: 0, // No rounding on left side
            bottomLeft: 0,
          };
          ...
          chamfer = {
            topLeft: 20,
            bottomLeft: 20,
            topRight: 0, // No rounding on right side
            bottomRight: 0,
          };
        }
      }
    ...
    if (context) {
      Events.on(render, "afterRender", () => {
        ...
        multipliers.forEach((multiplier, index) => {
          ...
          // Adjust font size based on multiplier value
          let fontSize;
          if (multiplier === "10x") {
            fontSize = 18; // Larger font for 10x
          } else if (multiplier === "5x" || multiplier === "2x") {
            fontSize = 16; // Medium font for 5x and 2x
          } else {
            fontSize = 14; // Smaller font for other values
          }
  ...
  return (
    <div className="min-h-screen bg-[url('/plinko.webp')] bg-cover bg-center flex flex-col p-4">
      ...
      <div className="flex flex-col lg:flex-row gap-4 flex-1">
        {/* Center - Game Board - Moved to top on mobile */}
        <div className="flex-1 flex flex-col items-center justify-center order-first lg:order-2">
        ...
        {/* Left Side Panel - Moved below game on mobile */}
        <div className="w-full lg:w-[350px] space-y-4 order-2 lg:order-1">
          {/* Mode Selector */}
        ...
        {/* Right Side Panel */}
        <div className="w-full lg:w-[350px] space-y-4 order-3">
          {/* Recent Bets */}
          <div className="bg-black/30 p-5 rounded-xl backdrop-blur-md border border-white/10 hover:border-white/20 transition-all">
